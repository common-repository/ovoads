...
class ExecuteRouter{
    ...
    public function execute()
    {
        ...
        foreach ($routes as $routeKey => $routeValue) {
            if (array_key_exists('uri',reset($routeValue))) {
                ...
                if (empty($params)) {
                    add_rewrite_rule($route['uri'], 'index.php?ovoads_page='.$route['uri'], 'top');
                }else{
                    add_rewrite_rule($exactUri.'$', 'index.php?ovoads_page='.str_replace('//','/',$uri).$paramMatch, 'top' );
                }
            }
        }
        flush_rewrite_rules();
    ...
    {
        if (ovoads_request()->page && ovoads_request()->module) {
            
            $action = ovoads_request()->module;
            $routes = Router::$routes;
            foreach ($routes as $routeKey => $routeValue) {
    ...
    public static function validateMethod($methodName)
    {
        if ($methodName != 'any') {
            ...
            if ($reqMethod != strtoupper($methodName)) {
                throw new \Exception(esc_html($reqMethod) . " method doesn't support for this route", 1);
            }
        }
    }
}